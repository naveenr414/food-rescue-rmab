...
# Analyze how synthetic transitions with variable parameters impact policy performances
...
if is_jupyter: 
    ...
    parser.add_argument('--max_transition_prob', help='Maximum Transition Prob from (0,0) to 1', type=float, default=0.25)
...
rewards, memory, simulator = run_multi_seed(seed_list,policy,results['parameters'],test_length=episode_len*(n_episodes%50),max_transition_prob=max_transition_prob)
...
if n_arms * volunteers_per_arm <= 4:
    ...
    rewards, memory, simulator = run_multi_seed(seed_list,policy,results['parameters'],per_epoch_function=per_epoch_function,test_length=episode_len*(n_episodes%50),max_transition_prob=max_transition_prob)
    results['{}_reward'.format(name)] = rewards['reward']
    results['{}_match'.format(name)] =  rewards['match'] 
    results['{}_active'.format(name)] = rewards['active_rate']
    results['{}_time'.format(name)] =  rewards['time']
...
if n_arms * volunteers_per_arm <= 1000:
    ...
    rewards, memory, simulator = run_multi_seed(seed_list,policy,results['parameters'],test_length=episode_len*(n_episodes%50),shapley_iterations=1000,max_transition_prob=max_transition_prob)
    results['{}_reward'.format(name)] = rewards['reward']
    results['{}_match'.format(name)] =  rewards['match'] 
    results['{}_active'.format(name)] = rewards['active_rate']
    results['{}_time'.format(name)] =  rewards['time']
...
if n_arms * volunteers_per_arm <= 25:
    ...
    rewards, memory, simulator = run_multi_seed(seed_list,policy,results['parameters'],test_length=episode_len*(n_episodes%50),mcts_test_iterations=400,max_transition_prob=max_transition_prob)
    results['{}_reward'.format(name)] = rewards['reward']
    results['{}_match'.format(name)] =  rewards['match'] 
    results['{}_active'.format(name)] = rewards['active_rate']
    results['{}_time'.format(name)] =  rewards['time']
