#!/bin/bash

# Main Scripts
bash ../bash_scripts/main_scripts/run_max.sh 
bash ../bash_scripts/main_scripts/run_linear.sh 
bash ../bash_scripts/main_scripts/run_set.sh 
bash ../bash_scripts/main_scripts/run_prob.sh 
bash ../bash_scripts/main_scripts/run_timing.sh 
bash ../bash_scripts/main_scripts/run_non_monotonic.sh
bash ../bash_scripts/main_scripts/food_rescue.sh
bash ../bash_scripts/main_scripts/run_contextual.sh
bash ../bash_scripts/main_scripts/run_two_timestep.sh

