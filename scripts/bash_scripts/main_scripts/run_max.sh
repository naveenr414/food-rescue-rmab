...
do
    ...
    do 
        ...
        do 
            ...
            for arm_set_high in 0.5 1 2 5 10 20
            do 
                tmux send-keys -t match_${session} "conda activate food; python all_policies.py --seed ${seed} --volunteers_per_arm 1 --n_arms ${n_arms} --lamb 0.5 --budget ${budget} --reward_type max --arm_set_low 0 --prob_distro custom_transition --arm_set_high ${arm_set_high} --out_folder reward_variation/max_reward" ENTER
                tmux send-keys -t match_${session} "conda activate food; python pure_rl.py --seed ${seed} --volunteers_per_arm 1 --n_arms ${n_arms} --lamb 0.5 --budget ${budget} --reward_type max --arm_set_low 0 --prob_distro custom_transition --arm_set_high ${arm_set_high} --out_folder baselines/all" ENTER
            done 
        done 
    done 
