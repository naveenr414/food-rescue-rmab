...
do
    ...
    do 
        ...
        do 
            ...
            tmux send-keys -t match_${session} "conda activate food; python all_policies.py --seed ${seed} --volunteers_per_arm 1 --n_arms ${n_arms} --lamb 0 --budget ${budget} --reward_type ${reward_type} --arm_set_low 0 --arm_set_high 1 --prob_distro ${prob_distro} --out_folder journal_results/contextual --n_episodes 5 --episode_len 50000 --discount 0.9999 2>&1 | tee ../../runs/contextual/${seed}_${n_arms}.txt" ENTER
        done 
    done 
done 