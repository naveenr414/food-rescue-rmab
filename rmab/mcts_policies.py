...
from rmab.omniscient_policies import greedy_policy, whittle_greedy_policy, whittle_index
...
def two_step_mcts_to_action(selected_idx,env,state,lamb, memory):    
    greedy_action, _ = greedy_policy(env,state,env.budget,lamb,None,None)
    ...
    # bin_counts = np.zeros(env.cohort_size, dtype=int)    
    # for i in selected_idx:
    #     bin_counts[i//env.volunteers_per_arm] += 1
...
def run_mcts(env,state,budget,lamb,memory,per_epoch_results,mcts_function,volunteer_class,idx_to_action,num_iterations=100):
    ...
    if num_iterations > 0:
        ...
        selected_idx = []
    action = idx_to_action(selected_idx,env,state,lamb,memory)
...
def mcts_policy(env,state,budget,lamb,memory,per_epoch_results):
    return run_mcts(env,state,budget,lamb,memory,per_epoch_results,mcts,VolunteerState,one_step_idx_to_action)
